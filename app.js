...
function dashboard() {
  ...
  const stockCost = lotAnalytics().reduce((sum, lot) => sum + [...lot.groups.values()].reduce((lotSum, group) => lotSum + group.cost, 0), 0);
  const liquidationResult = cashResult + stockCost;
...
function recordsView(kind) {
  ...
  const filteredRows = expense ? cfg.rows.filter(row => {
    const scopeOk = expenseReportFilters.scope === 'all' || (expenseReportFilters.scope === 'lot' ? Boolean(String(row.lot || '').trim()) : !String(row.lot || '').trim());
    const categoryOk = !expenseReportFilters.categories.length || expenseReportFilters.categories.includes(row.category);
    return scopeOk && categoryOk;
  }) : cfg.rows;
  const categories = expense ? [...new Set(cfg.rows.map(row => row.category).filter(Boolean))].sort((a,b) => a.localeCompare(b, 'pt-BR')) : [];
  const reportPanel = expense ? `<section class="panel expense-report"><div class="panel-head"><div><h3>Relatório de gastos</h3><small>Filtre gastos gerais ou vinculados a lote e selecione uma ou mais categorias.</small></div><button class="btn secondary" data-print-expense-report>Imprimir relatório</button></div><div class="expense-filter-grid"><label>Origem do gasto<select data-expense-scope><option value="all" ${expenseReportFilters.scope==='all'?'selected':''}>Todos os gastos</option><option value="general" ${expenseReportFilters.scope==='general'?'selected':''}>Somente gerais / sem lote</option><option value="lot" ${expenseReportFilters.scope==='lot'?'selected':''}>Somente vinculados a lote</option></select></label><div class="expense-category-filter"><strong>Categorias (selecione uma ou mais)</strong><div class="filter-options">${categories.map(category => `<label><input type="checkbox" data-expense-category value="${esc(category)}" ${expenseReportFilters.categories.includes(category)?'checked':''}> ${esc(category)}</label>`).join('')}</div></div></div><div class="summary-strip"><div>Itens filtrados<strong>${filteredRows.length}</strong></div><div>Pago<strong class="green">${money(total(filteredRows.filter(isExpensePaid)))}</strong></div><div>A pagar<strong class="red">${money(total(filteredRows.filter(row => !isExpensePaid(row))))}</strong></div><div>Total<strong>${money(total(filteredRows))}</strong></div></div><div class="sheet-actions"><button class="btn secondary" data-clear-expense-filter>Limpar filtros</button></div></section>` : '';
  return `<div class="content"><div class="section-title"><div><h2>${cfg.title}</h2><p>${cfg.desc}</p></div><div class="sheet-actions">${button(cfg.type)}</div></div>${summary}${reportPanel}<div class="panel"><div class="table-wrap"><table><thead><tr>${cfg.cols.map(c=>`<th>${c}</th>`).join('')}<th></th></tr></thead><tbody>${filteredRows.length ? filteredRows.map((row,index)=>`<tr>${cfg.values(row).map(value=>`<td>${value||'—'}</td>`).join('')}<td>${expense ? `<button class="btn ${isExpensePaid(row)?'danger':'primary'}" data-payment-toggle="${cfg.rows.indexOf(row)}">${isExpensePaid(row)?'Marcar a pagar':'Marcar pago'}</button> ` : ''}<button class="btn secondary" data-edit="${cfg.type}" data-index="${cfg.rows.indexOf(row)}">Editar</button> <button class="btn danger" data-delete="${cfg.type}" data-index="${cfg.rows.indexOf(row)}">Excluir</button></td></tr>`).join('') : `<tr><td class="empty" colspan="${cfg.cols.length+1}">Nenhum lançamento para os filtros selecionados.</td></tr>`}</tbody></table></div></div></div>`;
}
